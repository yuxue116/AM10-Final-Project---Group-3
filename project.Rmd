...
del_long <- del_merged %>% pivot_longer(cols=c(4:9),names_to="type",values_to="percent") %>% filter(date>=as.Date("2020-02-15"))
...
lon_long <- lon_long %>% mutate(month=month(date),day=day(date)) %>% merge(.,del_pm2_averages,by=c("month","day")) %>% mutate(per_change_pm25=(pm25-base_pm25)/base_pm25*100)
lon_long %>% 
  filter(date>=as.Date("2020-05-01")) %>% 
  ggplot(aes(y=per_change_pm25,x=percent)) +
  geom_smooth() + facet_wrap(~type,scales = "free")

zur_pm2_averages <- zur_merged %>% filter(year!=2020) %>% mutate(month=month(date),day=day(date)) %>% group_by(month,day) %>% summarise(base_pm25=mean(pm25))
zur_long <- zur_long %>% mutate(month=month(date),day=day(date)) %>% merge(.,del_pm2_averages,by=c("month","day")) %>% mutate(per_change_pm25=(pm25-base_pm25)/base_pm25*100)
zur_long %>% 
  filter(date>=as.Date("2020-05-01")) %>% 
  ggplot(aes(y=per_change_pm25,x=percent)) +
  geom_smooth() + facet_wrap(~type,scales = "free")
